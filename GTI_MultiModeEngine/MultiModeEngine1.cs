...
namespace GTI
{
    partial class GTI_MultiModeEngineOLD : PartModule
    {
        ...
        private List<engineMultiModeList> engineModeList = new List<engineMultiModeList>();
        ...
        public List<engineMultiModeList> GetengineModeList { get { return engineModeList; } }
        ...
        {
            GTIDebug.Log("GTI_MultiModeEngine : OnStart() : " + GTIDebug.GetVesselName(part), iDebugLevel.DebugInfo);
        ...
        {
            GTIDebug.Log("GTI_MultiModeEngine : OnStartFinished() : " + GTIDebug.GetVesselName(part), iDebugLevel.DebugInfo);
            updatePropulsion(silentUpdate: true);
        ...
        private void initializeSettings(bool loadConfigNode = false)
        {
            ...
            if (!_settingsInitialized)
            {
                ...
                GTIDebug.Log("GTI_MultiModeEngine - ConfigNode Before Load", iDebugLevel.DebugInfo);
                if (MultiModeConfigNode == null && loadConfigNode == true) { MultiModeConfigNode = part.GetPartModuleConfig("MODULE", "name", "GTI_MultiModeEngine"); GTIDebug.Log("\n" + MultiModeConfigNode.ToString(), iDebugLevel.DebugInfo); } else { GTIDebug.Log("Allready loaded\n" + MultiModeConfigNode.ToString(), iDebugLevel.Medium); }
                GTIDebug.Log("GTI_MultiModeEngine - ConfigNode Loaded", iDebugLevel.DebugInfo);
                ...
                ConfigNode[] atmosphereCurves =     loadConfigNode ? MultiModeConfigNode.GetNodes("atmosphereCurve")     : null;
                ConfigNode[] velCurves =            loadConfigNode ? MultiModeConfigNode.GetNodes("velCurve")            : null;
                ConfigNode[] atmCurves =            loadConfigNode ? MultiModeConfigNode.GetNodes("atmCurve")            : null;
                ConfigNode[] throttleISPCurves =    loadConfigNode ? MultiModeConfigNode.GetNodes("throttleISPCurve")    : null;
                ...
                {
                    engineModeList.Add(new engineMultiModeList()
                    {
                    ...
                    GTIDebug.Log("Parsing the usethrottleISPCurve", iDebugLevel.DebugInfo);
        ...
        private void updatePropulsion(bool silentUpdate = false)
        {
            ...
            if (!GTIthrottleISPCurvesEmpty) GTIDebug.Log("ISP Float Curve : " + engineModeList[selectedMode].GTIthrottleISPCurve.ToString(), iDebugLevel.DebugInfo); else GTIDebug.Log("no ISP Float Curve : " + GTIDebug.GetVesselName(part), iDebugLevel.DebugInfo);
            ...
            if (!EngineTypesEmpty) { ModuleEngines.engineType = GetEngineType(engineModeList[selectedMode].engineType); }
            #endregion
        ...
        {
            //node.AddValue("Tet", "TestValue", "TestComment");
            //node.SetValue()
        }
